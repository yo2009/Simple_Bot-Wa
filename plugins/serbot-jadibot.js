...
let rtx= `
*───❥͜🪴Pasos para escanear:*
\`\`\`1.- Toca los 3 botones de la esquina superior izquierda.
2.- Selecciona la opción "Vincular nuevo dispositivo" o "Linked Devices"
3.- Escanea este QR
...
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
  let parentw = conn
  if (conn.user.jid !== global.conn.user.jid) return parentw.reply(m.chat, '*🎋 Utiliza el comando en el bot principal:* wa.me/' + global.conn.user.jid.split`@`[0], m)
...
browser: ["The Eminence | Wa Subbot", "Firefox", "5.0"],
}
...
async function connectionUpdate(update) {
    ...
    console.log(await creloadHandler(true).catch(console.error))}
        ...
        parentw.sendMessage(m.chat, {text : "La conexión se cerró, Tendras que conectarte manualmente enviando el ID asignado...."}, { quoted: m })
    }
...
handler.tags = ['dl']
handler.command = ['jadibot', 'serbot', 'getcode']
